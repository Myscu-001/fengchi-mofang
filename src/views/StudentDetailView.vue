<template>
  <div>
    ...
    <div v-else class="fc-container py-7 space-y-5">
      ...
      <!-- 学习记录（入口，详情在子页面） -->
      <RouterLink
        :to="{ name: 'student-learning', params: { id: route.params.id } }"
        class="fc-card flex items-center gap-3.5 p-4.5 transition hover:-translate-y-0.5 hover:shadow-lift"
      >
        <span class="flex size-9 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-600">
          <BookOpen class="size-4" />
        </span>
        <div class="min-w-0 flex-1">
          <p class="flex items-center gap-2 text-[14px] font-semibold text-ink-900">
            学习记录
            <span
              v-if="learningLogs.length"
              class="rounded-md bg-ink-100 px-1.5 py-0.5 text-[11px] font-medium text-ink-600"
            >{{ learningLogs.length }} 条</span>
          </p>
          <p class="mt-0.5 line-clamp-1 text-[12.5px] text-ink-400">
            {{ logEntryHint }}
          </p>
        </div>
...
/** 学习记录入口卡的提示文案 */
const logEntryHint = computed(() => {
  if (learningMissing) return '数据表尚未创建，点进去查看说明'
  if (learningLoading.value) return '加载中…'
  if (!learningLogs.value.length) {
    return canManageStudent.value ? '还没有记录，点进去记一笔今天学了什么' : '暂无记录'
  }
  const latest = learningLogs.value[0]
  return `最近记录：${formatDate(latest.learned_on)} · 共 ${learningLogs.value.length} 条`
})
